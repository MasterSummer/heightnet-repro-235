...
def _aggregate_person_features(records: list[dict]) -> dict[str, dict[str, dict]]:
    """Group records by camera and person, keeping ALL frame-level features for voting."""
    ...
    for cam, persons in grouped.items():
        for pid, recs in persons.items():
            out[cam][pid] = {
                "features": [r["feature"] for r in recs],  # keep all frame features for voting
                "n_frames": len(recs),
    ...
    with torch.no_grad():
        for i in range(len(ids)):
            for j in range(i + 1, len(ids)):
                pid_i = ids[i]
                pid_j = ids[j]
                fi_all = person_features[pid_i]["features"]
                fj_all = person_features[pid_j]["features"]

                # Frame-level voting: compare all frame pairs, majority wins
                vote_i = 0
                vote_j = 0
                for fi in fi_all:
                    for fj in fj_all:
                        prob = torch.sigmoid(
                            model.compare_encoded(
                                fi.unsqueeze(0).to(device),
                                fj.unsqueeze(0).to(device),
                            )
                        )[0].item()
                        if prob >= 0.5:
                            vote_i += 1
