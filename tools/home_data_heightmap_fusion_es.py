...
VARIANTS = (
    ...
    "fused_concat",
    "bbox_film_crop",
    "bbox_quality_gated_fusion",
    "bbox_crop_cross_attn",
...
class EarlyStopper:
    ...
    @staticmethod
    def key(metrics: dict) -> tuple[float, float, float, int, int]:
        spearman_val = metrics.get("spearman")
        bisect_val = metrics.get("bisect", 0.0)
        if isinstance(bisect_val, dict):
            bisect_val = bisect_val.get("hit_rate") or 0.0
        return (
            float(metrics.get("val_pairwise") or 0.0),
            -1e9 if spearman_val is None else float(spearman_val),
            float(bisect_val or 0.0),
            -int(metrics.get("epoch") or 0),
            -int(metrics.get("seed") or 0),
        )

    def update(self, metrics: dict) -> bool:
        key = self.key(metrics)
        improved = self.best_key is None or key[0] >= self.best_key[0] + self.min_delta
        if improved:
            self.best_key = key
            self.bad_count = 0
            self.should_stop = False
            return True
        self.bad_count += 1
        self.should_stop = self.bad_count >= self.patience
        return False


def select_best_seed_result(results: list[dict]) -> dict:
    return max(results, key=lambda r: (
        float(r["val_pairwise"]),
        -1e9 if r["val"].get("spearman") is None else float(r["val"].get("spearman")),
        float(r["val"].get("bisect", {}).get("hit_rate") or 0.0),
        -int(r["best_epoch"]),
        -int(r["seed"]),
    ))
...
def write_loss_history(path: Path, rows: list[dict]) -> None:
    ...
    cols = ["camera_id", "variant", "seed", "epoch", "split", "total_loss", "mse_loss", "pair_loss"]
...
def plot_loss_history(rows: list[dict], out_dir: Path, variant: str) -> None:
    if not rows:
        return
    out_dir.mkdir(parents=True, exist_ok=True)
    cameras = sorted({r["camera_id"] for r in rows})
    for cam in cameras:
        _plot_one_loss([r for r in rows if r["camera_id"] == cam], out_dir / f"{cam}_{variant}_loss.png", f"{cam} {variant}")
    _plot_one_loss(rows, out_dir / f"{variant}_macro_loss.png", f"{variant} macro")


def _plot_one_loss(rows: list[dict], path: Path, title: str) -> None:
    grouped: dict[tuple[str, str], dict[int, list[float]]] = {}
    for r in rows:
        key = (str(r["split"]), "total_loss")
        grouped.setdefault(key, {}).setdefault(int(r["epoch"]), []).append(float(r["total_loss"]))
    plt.figure(figsize=(8, 5))
    for (split, _), by_epoch in sorted(grouped.items()):
        xs = sorted(by_epoch)
...
class FusionRegressor(nn.Module):
    def __init__(self, variant: str, tabular_dim: int = 9, hidden: int = 128, dropout: float = 0.1):
        ...
        if variant == "bbox_only":
        ...
        elif self.variant == "bbox_film_crop":
            self.tabular = TabularMLP(tabular_dim, hidden, dropout)
            self.crop = CropResNetEncoder(max(16, hidden // 2))
            self.film = nn.Linear(self.tabular.out_dim, self.crop.out_dim * 2)
            head_in = self.crop.out_dim
        elif self.variant == "bbox_quality_gated_fusion":
            self.tabular = TabularMLP(tabular_dim, hidden, dropout)
            self.crop = CropResNetEncoder(max(16, hidden // 2))
            self.bbox_score = nn.Linear(self.tabular.out_dim, 1)
            self.crop_score = nn.Linear(self.crop.out_dim, 1)
            self.gate = nn.Sequential(
                nn.Linear(self.tabular.out_dim + self.crop.out_dim, hidden),
                nn.ReLU(inplace=True),
                nn.Linear(hidden, 1),
                nn.Sigmoid(),
            )
            self.last_gate = None
            head_in = 1
        elif self.variant == "bbox_crop_cross_attn":
            self.tabular = TabularMLP(tabular_dim, hidden, dropout)
            self.crop = CropResNetEncoder(hidden)
            self.crop_proj = nn.Linear(self.crop.out_dim, self.tabular.out_dim)
            self.attn = nn.MultiheadAttention(self.tabular.out_dim, num_heads=4, batch_first=True)
            head_in = self.tabular.out_dim
        else:
            self.tabular = TabularMLP(tabular_dim, hidden, dropout)
            self.crop = CropResNetEncoder(max(16, hidden // 2))
    ...
    def forward(self, tabular, crop):
        if self.variant == "bbox_only":
            ...
            crop_feat = self.crop(crop)
            scale, shift = self.film(tab).chunk(2, dim=1)
            feat = crop_feat * (1.0 + torch.tanh(scale)) + shift
        elif self.variant == "bbox_quality_gated_fusion":
            tab = self.tabular(tabular)
            crop_feat = self.crop(crop)
            gate = self.gate(torch.cat([tab, crop_feat], dim=1))
            self.last_gate = gate.detach()
            bbox_score = self.bbox_score(tab)
            crop_score = self.crop_score(crop_feat)
            return (gate * crop_score + (1.0 - gate) * bbox_score).squeeze(-1)
        elif self.variant == "bbox_crop_cross_attn":
            tab = self.tabular(tabular)
            crop_feat = self.crop_proj(self.crop(crop))
            tokens = torch.stack([tab, crop_feat], dim=1)
            attended, _ = self.attn(tokens, tokens, tokens, need_weights=False)
            feat = attended.mean(dim=1)
        else:
...
def train_camera(
    cam, ids, meta, tabular, crops, person_heights, device, epochs, seeds, hidden, variant,
    ckpt_dir, limit_per_camera=None, pair_loss_weight=0.5, near_bucket_weights=None,
    eval_every=20, patience=10, min_delta=0.002,
):
    ...
    for seed in seeds:
        ...
        for ep in range(1, epochs + 1):
            ...
            if ep % max(1, eval_every) == 0 or ep == epochs:
                model.eval()
                with torch.no_grad():
                    pred_eval = model(x, c)
                    pp = pred_eval.detach().cpu().numpy()
                ...
                val_sp = spearman([scores[s] for s in val_ids], [heights[s] for s in val_ids]) if len(val_ids) >= 2 else None
                val_bisect = bisect(val_ids, scores, heights)
                ...
                for split_name in ("train", "val"):
                    sids = by_split[split_name]
                    split_idx = torch.tensor([sid_to_idx[s] for s in sids], device=device, dtype=torch.long)
                    split_mse = torch.nn.functional.mse_loss(pred_eval[split_idx], yy[split_idx]) / 1000.0
                    split_pair_loss = _pair_loss_for_ids(sids, pred_eval, sid_to_idx, heights, bucket_weights, device)
                    split_total = split_mse + float(pair_loss_weight) * split_pair_loss
                    loss_history.append({
                        "camera_id": cam,
                        "variant": variant,
                        "seed": seed,
                        "epoch": ep,
                        "split": split_name,
                        "total_loss": float(split_total.detach().cpu()),
                        "mse_loss": float(split_mse.detach().cpu()),
                        "pair_loss": float(split_pair_loss.detach().cpu()),
                    })
    ...
    return {
        ...
        "early_stopping": {"eval_every": int(eval_every), "patience": int(patience), "min_delta": float(min_delta)},
...
def _pair_loss_for_ids(sids, pred, sid_to_idx, heights, bucket_weights, device):
    pairs = []
    ...
    idx = torch.tensor([[a, b] for a, b, _, _ in pairs], device=device, dtype=torch.long)
    sign = torch.tensor([s for _, _, s, _ in pairs], device=device, dtype=torch.float32)
    weight = torch.tensor([w for _, _, _, w in pairs], device=device, dtype=torch.float32)
...
def summarize_camera_results(camera_results: dict) -> dict:
    ...
    for item in camera_results.values():
        if item.get("skipped"):
            continue
        test = item["best"]["test"]
        if test.get("pairwise_accuracy") is not None:
            pairaccs.append(float(test["pairwise_accuracy"]))
        if test.get("spearman") is not None:
            spearmans.append(float(test["spearman"]))
        counts = test.get("pairwise_counts") or {}
        correct += int(counts.get("correct") or 0)
        total += int(counts.get("total") or 0)
    pairaccs_sorted = sorted(pairaccs)
    if not pairaccs_sorted:
        median = None
    elif len(pairaccs_sorted) % 2:
        median = pairaccs_sorted[len(pairaccs_sorted) // 2]
    else:
        mid = len(pairaccs_sorted) // 2
        median = (pairaccs_sorted[mid - 1] + pairaccs_sorted[mid]) / 2.0
    return {
        "n_cameras": len(pairaccs),
        "macro_pairacc": float(np.mean(pairaccs)) if pairaccs else None,
        "micro_pairacc": correct / total if total else None,
        "median_pairacc": median,
        "macro_spearman": float(np.mean(spearmans)) if spearmans else None,
        "pairwise_counts": {"correct": correct, "total": total, "accuracy": correct / total if total else None},
    }
...
def main():
    ...
    feature_payload = {
        ...
        "early_stopping": {"eval_every": args.eval_every, "patience": args.patience, "min_delta": args.min_delta},
    }
    ...
    results = {
        ...
        "pair_loss_weight": args.pair_loss_weight,
        "near_bucket_weights": near_bucket_weights,
        "checkpoint_selection": "validation-only early stopping: val PairAcc, val Spearman, val bisect, earlier epoch, smaller seed",
        "frame_input": "all_frames_mean: NPZ bbox_feats/height_stats/heightmap_crops are averaged across all frames; no max-frame truncation",
    ...
    for cam in cameras:
        ...
        results["camera_results"][cam] = train_camera(
            ...
            args.pair_loss_weight, near_bucket_weights, args.eval_every, args.patience, args.min_delta,
        )
    all_loss_rows = []
    for cam_result in results["camera_results"].values():
        all_loss_rows.extend(cam_result.get("loss_history", []))
    loss_path = out_dir / f"loss_history_{args.variant}.csv"
    write_loss_history(loss_path, all_loss_rows)
    if args.plot_loss:
        plot_loss_history(all_loss_rows, out_dir / "plots", args.variant)
    results["summary"] = summarize_camera_results(results["camera_results"])
    result_path = out_dir / f"base_camera_heightmap_fusion_es_{args.variant}_results.json"
