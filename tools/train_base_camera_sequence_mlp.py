...
def main():
    ...
    ap.add_argument('--out', default='/data1/zyding/jianzhi_2511_base_camera_rect_sequence/base_camera_mlp_results.json')
    ...
    # Add video-level rankings for every base camera and seed-best model output.
    for cam,res in out['camera_results'].items():
        ...
        res['video_level_ranking']=[{
            ...
            'split': SPLIT.get(meta[sid]['person_id'], ''), 'height_cm': float(heights.get(meta[sid]['person_id'], 0.0)), 'score': float(scores[sid])
