...
VARIANTS = ("temporal_bbox_only", "temporal_crop_only", "late_gated_fusion", "film_attn_fusion")
LOSS_FIELDS = [
    "camera_id",
    "variant",
    "seed",
...
class FrameCropEncoder(nn.Module):
    def __init__(self, out_dim: int):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(1, 12, 5, stride=2, padding=2, bias=False),
            nn.GroupNorm(1, 12),
            nn.ReLU(inplace=True),
            ResidualBlock(12),
...
class TemporalFusionRanker(nn.Module):
    def __init__(self, variant: str, hidden: int = 128, n_bins: int = 16):
        super().__init__()
        ...
        self.crop = FrameCropEncoder(hidden)
        self.film = FiLMConditioner(hidden, hidden)
        self.attn_bbox = MaskedAttentionPool(hidden)
        self.attn_crop = MaskedAttentionPool(hidden)
        self.attn_cond = MaskedAttentionPool(hidden)
        self.fusion = GatedFusion(hidden, 3)
    ...
    def forward(self, tabular, crops, mask):
        ...
        cond_feat = self.film(crop_feat, bbox_feat)
        attention_weights = {}
        if self.variant == "temporal_bbox_only":
            pooled, w = self.attn_bbox(bbox_feat, mask)
            attention_weights["bbox"] = w
        elif self.variant == "temporal_crop_only":
            pooled, w = self.attn_crop(crop_feat, mask)
            attention_weights["crop"] = w
        elif self.variant == "late_gated_fusion":
            bbox_pool, wb = self.attn_bbox(bbox_feat, mask)
            crop_pool, wc = self.attn_crop(crop_feat, mask)
            cond_pool = (bbox_pool + crop_pool) * 0.5
            pooled, gate = self.fusion([bbox_pool, crop_pool, cond_pool])
            attention_weights = {"bbox": wb, "crop": wc, "gate": gate}
        else:
            bbox_pool, wb = self.attn_bbox(bbox_feat, mask)
            crop_pool, wc = self.attn_crop(crop_feat, mask)
            cond_pool, wf = self.attn_cond(cond_feat, mask)
            pooled, gate = self.fusion([bbox_pool, crop_pool, cond_pool])
            attention_weights = {"bbox": wb, "crop": wc, "conditioned": wf, "gate": gate}
        return {
...
def compute_losses(outputs, heights, ordinal_y, pair_loss_weight, ordinal_loss_weight, near_bucket_weights):
    ...
    total = mse + pair + float(ordinal_loss_weight) * ordinal
    return total, mse, pair, ordinal
...
def evaluate_loss(model, ids, sid_to_idx, x, c, mask, y, ordinal_y, device, batch_size, pair_loss_weight, ordinal_loss_weight, near_bucket_weights):
    if not ids:
        return {"total_loss": 0.0, "mse_loss": 0.0, "pair_loss": 0.0, "ordinal_loss": 0.0}
    model.eval()
    totals = []
    mses = []
    pairs = []
    ...
    with torch.no_grad():
        for batch_ids in iter_batches(ids, batch_size, False, random.Random(0)):
            ...
            out = model(xx, cc, mm)
            total, mse, pair, ordinal = compute_losses(out, yy, oo, pair_loss_weight, ordinal_loss_weight, near_bucket_weights)
...
def train_camera(cam, ids, meta, person_heights, device, epochs, seeds, hidden, variant, ckpt_dir, limit_per_camera, pair_loss_weight, ordinal_loss_weight, near_bucket_weights, batch_size, log_loss_every):
    ...
    for seed in seeds:
        ...
        model = TemporalFusionRanker(variant=variant, hidden=hidden, n_bins=16).to(device)
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch_ids in iter_batches(by_split["train"], batch_size, True, rng):
                ...
                total.backward()
                opt.step()
                train_losses.append((float(total.detach().cpu()), float(mse.detach().cpu()), float(pair.detach().cpu()), float(ordinal.detach().cpu())))
...
def main():
    ...
    ap.add_argument("--variant", choices=VARIANTS, default="film_attn_fusion")
    ...
    result_path = out_dir / f"base_camera_heightmap_fusion_v2_{args.variant}_results.json"
